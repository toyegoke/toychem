...
def RDS(moleculer_property_function, title_of_molecule, Order_specieID_list, rtime, rtemp, cmass, flowout,initial_coverage):      # <=============  
    ...
    mc0 = cmass #40  # in g
    C_R0 = 100 # 997.74 # mol/m3 
    outflow_rate = flowout * 0.5e-3 # (14.27) * 1e-7 # in m3/sec
    v_R0 = 0.5 # in m3 (45 mL)
    ...
    S_x = 6e-7# mol/m^2 (equivalent of 0.44 ML from expt data)
    ...
    def RDS_effect2(moleculer_property_function, title_of_molecule, Order_specieID_list, fat, bat, gat, dfff):      # <=============    
        CCL = []        #RR = []
        RP = []; RPP = []
        RH = []; RHH = []
        T = temp
            
        tt0, y= solve_odes46cc(SA,n_R0,v_R0,C_R0,outflow_rate,temp,TIME,dydtc, G_m, int_id, Order_specieID_list,S_x, fat, bat, gat, initial_coverage)
        #tt0, y= solve_odes46c(SA,n_R0,v_R0,C_R0,outflow_rate,temp,TIME,dydtc, G_m, int_id, Order_specieID_list,S_x, fat, bat, gat)
        ...
        ax.semilogx(xx1, y[:,7]/y[0,6], label='$n_{H2}$')
        ax.semilogx(xx1, y[:,8]/y[0,6], label='$n_P$')
        #plt.title(title+' Production Profile')
        plt.xlabel('Time, $ t/sec $')
        plt.ylabel('Relative Amount, $ {^{n_i}/_{n_{R,0}}} $')
        ax.legend()
        fig.savefig('/home/toyegoke/ENS_kinetics/Final-RDS-G/'+title+'_'+str(dfff)+'_state_'+'_1amount_plot_.png')

        fig = plt.figure()
        ay = plt.subplot(111)
        Sx = y[:,0]+y[:,1]+y[:,2]+y[:,3]+y[:,4]+y[:,5]
        ay.semilogx(xx1, y[:,0]/Sx, label='$\Theta_X$')
        ay.semilogx(xx1, y[:,1]/Sx, label='$\Theta_{RX}$')
        ay.semilogx(xx1, y[:,2]/Sx, label='$\Theta_{UX}$')
        ay.semilogx(xx1, y[:,3]/Sx, label='$\Theta_{HX}$')
        ay.semilogx(xx1, y[:,4]/Sx, label='$\Theta_{VX}$')
        ay.semilogx(xx1, y[:,5]/Sx, label='$\Theta_{PX}$')
        #plt.title(title+' Coverage Profile')
        plt.xlabel('Time, $ t/sec $')
        plt.ylabel('Relative Coverage, $ {^{\Theta_i}/_{\Theta_{X,0}}} $')   
        ay.legend()  
        fig.savefig('/home/toyegoke/ENS_kinetics/Final-RDS-G/'+title+'_'+str(dfff)+'_state_'+'_1surface_plot_.png')
        #========================================
        ...
        #RPP.append(rP)  #rateP= -outflow_rate/v_R0*(0-n_P)
        #RHH.append(rH)  # rateH2= -outflow_rate/v_R0*(0-n_H2)
    ...
    def gat_RDS2(title_of_molecule, moleculer_property_function, Order_specieID_list):
        ...
        #dff = [-6000,-4000,-2000,0,2000,4000,6000]
        #dff = [-3000,-2000,-1000,0,1000,2000,3000]
        ...
        for i in range(rangedff):
            ...
            RH1, RP1, lnRH1, lnRP1, gatG1 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat1, 'RX'+str(i))
            RH2, RP2, lnRH2, lnRP2, gatG2 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat2, 'UX'+str(i))
            RH3, RP3, lnRH3, lnRP3, gatG3 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat3, 'HX'+str(i))
            RH4, RP4, lnRH4, lnRP4, gatG4 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat4, 'VX'+str(i))
            RH5, RP5, lnRH5, lnRP5, gatG5 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat5, 'PX'+str(i))
            RH6, RP6, lnRH6, lnRP6, gatG6 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat6, 'TS1'+str(i))
            RH7, RP7, lnRH7, lnRP7, gatG7 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat7, 'TS2'+str(i))
            RH8, RP8, lnRH8, lnRP8, gatG8 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat8, 'TSR'+str(i))
            RH9, RP9, lnRH9, lnRP9, gatG9 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat9, 'TSP'+str(i))
            RH0, RP0, lnRH0, lnRP0, gatG0 = RDS_effect2(ppt, title0, Order_specieID_list, fat, bat, gat0, 'TSH2'+str(i))
...
cmass = 750 # 1e-3 # mg
flowout = 1 # semi-batch/CSTR is 1 and batch is 0
...
rtemp = [550,550,550,550,550] # 550K
TIM = 3600*1 # in sec
rtime = [TIM,TIM,TIM,TIM,TIM] # in sec # rtime = [10**(5),10**(5),10**(5),10**(5),10**(5)] #for 5%/20%
