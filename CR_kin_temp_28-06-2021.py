...
#=====================================================================================================
#=====================================================================================================
def solve_odes46cc(SA, n_R0,v_R0,C_R0,outflow_rate,T,tim,dydt,Gm,intID,specieID_list, S_x, fat, bat, gat, initial_coverage):
    """
    ...
    v_initial = v_R0*1  # m3, volume the reactor/batch when it is dependent on temp (T)
    ...
    #y0 = [S_x0*1, S_x0*0.00, S_x0*0.00, S_x0*0.00, S_x0*0.00, S_x0*0.00,  n_R0,  0,  0, 0] 
    #y0 = [S_x0*1, S_x0*0, S_x0*0, S_x0*0, S_x0*0,  S_x0*0,  n_R0,  0,  0]  
    #       X,     RX,     UX,       HX,      Vx,      PX,    R,   Hin, P
...
def solve_odes46c(SA, n_R0,v_R0,C_R0,outflow_rate,T,tim,dydt,Gm,intID,specieID_list, S_x, fat, bat, gat):
    """
    Time-integrate chemo-kinetic system
    mc0 (gram)e.g.25, C_R0 (mol/m3)e.g.950, outflow_rate(m3/s)e.g. 5e-7, 
    Temp(K), time(sec), dydt, Energylist, intID_list, Order_specieID_list
...
def dydtc(t, y, params):
    ...
    """
    # case 2 when gas specie is in n, mol.  
    rf1 = kf1 * n_R * X
    rb1 = kb1 * RX  
    rf2 = kf2 * RX * X  
    rb2 = kb2 * UX * HX  
    ...
    rb3 = kb3 * PX * HX  
    rf4 = kf4 * PX  
    rb4 = kb4 * n_P/v_R0 * X /1e-6
    rf5 = kf5 * HX**2   
    rb5 = kb5 * n_H2/v_R0 * (X**2) /1e-6
    rf6 = kf6 * VX  
    rb6 = kb6 * PX
    ...
    dydt[0] = 1*(-rf1 + rb1 - rf2 + rb2 - rf3 + rb3 + rf4 - rb4  + 2*rf5 - 2*rb5)*au # X in umol/m2/sec
    dydt[1] = 1*(rf1 - rb1 - rf2 + rb2)*au # RX in umol/m2/sec
    dydt[2] = 1*(rf2 - rb2 - rf3 + rb3)*au # UX in umol/m2/sec
    dydt[3] = 1*(rf2 - rb2 + rf3 - rb3 - 2*rf5 + 2*rb5)*au # HX in umol/m2/sec
    dydt[4] = 1*(-rf6 + rb6)*au # VX in umol/m2/sec
    dydt[5] = 1*(rf3 - rb3 - rf4 + rb4 + rf6 - rb6)*au # PX in umol/m2/sec

    # ODEs for rate of change in gas-phase concentration (CSTR reactor model)
    n_out = 0 # mol, amount of R charged out at time, t 

    dydt[6] = SA*1e-6*(-rf1 + rb1)*au + 1*outflow_rate/v_batch*(nR0 - n_R) #n_out) # R in mol/sec 
    dydt[7] = SA*1e-6*(rf5 - rb5)*au + 1*outflow_rate/v_batch*(n_in - n_H2)  # H2 in mol/sec
    dydt[8] = SA*1e-6*(rf4 - rb4)*au + 1*outflow_rate/v_batch*(n_in - n_P)  # P in mol/sec
    #dydt[9] = SA*1e-6*(rf4 - rb4)*au 
