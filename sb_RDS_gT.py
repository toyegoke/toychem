...
# GOOD FOR RATE-DETERMING-STEP CALCULATION /// ON THE KINETICS RATE FOR DIFF CASES // for ENERGY STATE

#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

# TYPE 2 FOR RDS WITH TEMP EFFECT

#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

def RDStemp(moleculer_property_function, title_of_molecule, Order_specieID_list, reactiontime, reactiontemp, catmass, flowout):      # <=============  
    ...
    filename_link_fomat = "/home/toyegoke/ENS_kinetics/Final-RDS-Gtemp/"+title+'_'+"_THERMO_data_.csv"     #+str(T)
    filename_link_fomat2 = "/home/toyegoke/ENS_kinetics/Final-RDS-Gtemp/"+title+'_'+"_PES_profile_data_.csv"  #+str(T)

    G_m, int_id, S_m, H_m, fig, fig2 = ppt(temp, filename_link_fomat, filename_link_fomat2)

    fig2.savefig('/home/toyegoke/ENS_kinetics/Final-RDS-Gtemp/'+title+'_'+'_plot_data_for_ENERGY_PROFILE.png')  #  +str(T)  
    fig.savefig('/home/toyegoke/ENS_kinetics/Final-RDS-Gtemp/'+title+'_'+'_plot_data_for_COMBINED_ENERGY_PROFILE.png')  # +str(T)
    ...
    def RDS_effect2(moleculer_property_function, title_of_molecule, Order_specieID_list, fat, bat, gat):      # <=============    
        ...
        ax.legend()
        fig.savefig('/home/toyegoke/ENS_kinetics/Final-RDS-Gtemp/'+title+'_'+str(int(T))+'K_'+'_1amount_plot_.png')
        ...
        ay.legend()  
        fig.savefig('/home/toyegoke/ENS_kinetics/Final-RDS-Gtemp/'+title+'_'+str(int(T))+'K_'+'_1surface_plot_.png')
        #========================================
        ...
        gatG = [RXg,UXg,HXg,VXg,PXg,TS1g,TS2g]
        print(gatG)    
    ...
    def gat_RDS2(title_of_molecule, moleculer_property_function, Order_specieID_list, temp):
        ...
        for i in range(rangedff):
            ...
            gat1=[dff[i],0,0,0,0,0,0] 
            gat2=[0,dff[i],0,0,0,0,0] 
            gat3=[0,0,dff[i],0,0,0,0] 
            gat4=[0,0,0,dff[i],0,0,0] 
            gat5=[0,0,0,0,dff[i],0,0] 
            gat6=[0,0,0,0,0,dff[i],0] 
            gat7=[0,0,0,0,0,0,dff[i]] 
        ...
        lnG=[lnG1,lnG2,lnG3,lnG4,lnG5,lnG6,lnG7]; gaps=dff
        print(lnG)
        return hhh1,hhh2,hhh3,hhh4,hhh5,hhh6,hhh7,ppp1,ppp2,ppp3,ppp4,ppp5,ppp6,ppp7,lnG,gaps
    ...
    def RDStempsolve(moleculer_property_function,title_of_molecule,Order_specieID_list,temp):
        ...
        for i in range(tlens):
            T=TT[i]
            hhh1,hhh2,hhh3,hhh4,hhh5,hhh6,hhh7,ppp1,ppp2,ppp3,ppp4,ppp5,ppp6,ppp7,dff,gaps =gat_RDS2(ppt,title,engg,T)
            ...
            m1.append(slope1); m2.append(slope2); m3.append(slope3); m4.append(slope4); m5.append(slope5); m6.append(slope6); m7.append(slope7)
            c1.append(intercept1); c2.append(intercept2); c3.append(intercept3); c4.append(intercept4)
            c5.append(intercept5); c6.append(intercept6); c7.append(intercept7)
            
            m11.append(slope11); m22.append(slope22); m33.append(slope33); m44.append(slope44); m55.append(slope55)
            m66.append(slope66); m77.append(slope77)
            c11.append(intercept11); c22.append(intercept22); c33.append(intercept33); c44.append(intercept44)
            c55.append(intercept55); c66.append(intercept66); c77.append(intercept77)
    
        fig = plt.figure()
        ax = plt.subplot(111)
        gaps = TT
        ax.plot(gaps,m11,marker='o',linestyle='--', label='RX')
        ax.plot(gaps,m22,marker='x',linestyle='-.', label='UX')
        ax.plot(gaps,m33,marker='v',linestyle='--', label='HX')
        ax.plot(gaps,m44,marker='^',linestyle='-.', label='VX')
        ax.plot(gaps,m55,marker='+',linestyle='--', label='PX')
        ax.plot(gaps,m66,marker='x',linestyle='-.', label='TS1')
        ax.plot(gaps,m77,marker='x',linestyle='-.', label='Ts2')
        plt.xlabel('$T / K$')
        plt.ylabel('$X_{TRC}$')
        plt.title('TRC for '+title+' Kinetics')
        ax.legend()
        #plt.show()
        fig.savefig('/home/toyegoke/ENS_kinetics/Final-RDS-Gtemp/'+title+'_plot_data_for_TRC_Gtemp(H2_G).png')
    
        fig = plt.figure()
        ay = plt.subplot(111)
        gaps = TT
        ay.plot(gaps,m1,marker='o',linestyle='--', label='RX')
        ay.plot(gaps,m2,marker='x',linestyle='-.', label='UX')
        ay.plot(gaps,m3,marker='v',linestyle='--', label='HX')
        ay.plot(gaps,m4,marker='^',linestyle='-.', label='VX')
        ay.plot(gaps,m5,marker='+',linestyle='--', label='PX')
        ay.plot(gaps,m6,marker='x',linestyle='-.', label='TS1')        
        ay.plot(gaps,m7,marker='x',linestyle='-.', label='TS2')
        plt.xlabel('$T / K$')
        plt.ylabel('$X_{TRC}$')
        plt.title('TRC for '+title+' Kinetics')
        ay.legend()
        #plt.show()
        fig.savefig('/home/toyegoke/ENS_kinetics/Final-RDS-Gtemp/'+title+'_plot_data_for_TRC_Gtemp(P_G).png')
    
        csvfile = "/home/toyegoke/ENS_kinetics/Final-RDS-Gtemp/"+title+"_data_TRC_G_TEMP_.csv"
