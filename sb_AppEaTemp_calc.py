...
def EaApp(moleculer_property_function, title_of_molecule, Order_specieID_list, rtime, rtemp, cmass, flowout):      # <=============  
    ...
    def EA_effect(moleculer_property_function, title_of_molecule, Order_specieID_list, fat, bat, gat):      # <=============  
        ...
        RP = []
        RH = []
        #TT = [temp-20,temp-16,temp-12,temp-8,temp-4,temp,temp+4,temp+8,temp+12,temp+16,temp+20] # USING +/- 20kJ/mol
        TT = [temp-10,temp-8,temp-6,temp-4,temp-2,temp,temp+2,temp+4,temp+6,temp+8,temp+10] # USING +/- 10K
        tlens = len(TT)
        i = 0
        ...
        for i in range(tlens):
            ...
            filename_link_fomat = "/home/toyegoke/ENS_kinetics/Final-EaT/"+title+'_'+"_THERMO_data_.csv"     #+str(T)
            filename_link_fomat2 = "/home/toyegoke/ENS_kinetics/Final-EaT/"+title+'_'+"_PES_profile_data_.csv"  #+str(T)
        
            G_m, int_id, S_m, H_m, fig, fig2 = ppt(T, filename_link_fomat, filename_link_fomat2)
        
            fig.savefig('/home/toyegoke/ENS_kinetics/Final-EaT/'+title+'_'+'_plot_data_for_COMBINED_ENERGY_PROFILE.png')  # +str(T)
            fig2.savefig('/home/toyegoke/ENS_kinetics/Final-EaT/'+title+'_'+'_plot_data_for_ENERGY_PROFILE.png')  #  +str(T)          
            
            tt0, y= solve_odes40(SA,n_R0,v_R0,C_R0,outflow_rate,T,TIME,dydt, G_m, int_id, Order_specieID_list,S_x, fat, bat, gat)
        ...
        return tempt, RH, RP, loge(RH), loge(RP)                       #float(hh[-1]/rr[0]), float(pp[-1]/rr[0])    
...
for i in range(ranP):
    ...
    Eaa2=[]
    TT=rtemp[i]
    TEMP=[TT-50,TT-25,TT,TT+25,TT+50]
    rxnTime=rtime[i]

    for j in range(ranP):
        T=TEMP[j]
        AppEa1, AppEa2 = EaApp(ppt,title,engg,rxnTime,T,cmass,flowout)
        Eaa1.append(AppEa1)  
        Eaa2.append(AppEa2)          

    fig = plt.figure()
    ax = plt.subplot(111)
    ax.plot(TEMP, Eaa1, label=title) #marker='o',
    plt.xlabel('$T / K$')
    plt.ylabel('$Ea / kJ/mol$')
    plt.title(title+' Dehydrogenation')
    ax.legend()
    fig.savefig('/home/toyegoke/ENS_kinetics/Final-EaT/'+title+'_plot_for_Ea(T,H2).png')

    fig = plt.figure()
    ax = plt.subplot(111)
    ax.plot(TEMP, Eaa2, label=title) #marker='o',
    plt.xlabel('$T / K$')
    plt.ylabel('$Ea / kJ/mol$')
    plt.title(title+' Dehydrogenation')
    ax.legend()
    fig.savefig('/home/toyegoke/ENS_kinetics/Final-EaT/'+title+'_plot_for_Ea(T,P).png')
