...
#=====================================================================================================
#=====================================================================================================
# RATE CONSTANTS CALCULATIONS (SEMIBATCH)
#=====================================================================================================
#=====================================================================================================
...
def solve_odes(T,tim,dydt,Gm,intID,specieID_list, S_x):
    ...
    v_initial = v_R0  # m3, volume the reactor/batch when it is dependent on temp (T)
...
def solve_odes40(SA, n_R0,v_R0,C_R0,outflow_rate,T,tim,dydt,Gm,intID,specieID_list, S_x, fat, bat, gat):
    ...
    # integrate on a logaritmic scale
    # xx = np.linspace(0, 1000, 100)
    xx = np.linspace(0, np.log10(t1), 20)# old was 2)#
    yy = []
    tt = []
    time= []
...
#=====================================================================================================
#=====================================================================================================
def solve_odes45(SA, n_R0,v_R0,C_R0,outflow_rate,T,tim,dydt,Gm,intID,specieID_list, S_x, fat, bat, gat):
    """
    Time-integrate chemo-kinetic system
    mc0 (gram)e.g.25, C_R0 (mol/m3)e.g.950, outflow_rate(m3/s)e.g. 5e-7, 
    Temp(K), time(sec), dydt, Energylist, intID_list, Order_specieID_list
    ...
    # Initial conditions
    S_x0 = S_x
    y0 = [0, S_x0*1, S_x0*0, S_x0*0, S_x0*0, S_x0*0, n_R0, 0, 0,    0]
    t0 = 0
    t1 = tim # total integration time in sec
    ...
    # integrate on a logaritmic scale
    #xx = np.linspace(0, t1, 3)
    xx = np.linspace(0, np.log10(t1), 3)
    #xx = np.linspace(-14.0, np.log10(t1), int((np.log10(t1-200) + 24.0) * 100)) #(-14, np.log10(t1), int((np.log10(t1-500)+100)*200)) #int(t1*200))  #t1), 600)#
...
def solve_odes46(SA, n_R0,v_R0,C_R0,outflow_rate,T,tim,dydt,Gm,intID,specieID_list, S_x, fat, bat, gat):
    """
    Time-integrate chemo-kinetic system
    mc0 (gram)e.g.25, C_R0 (mol/m3)e.g.950, outflow_rate(m3/s)e.g. 5e-7, 
    Temp(K), time(sec), dydt, Energylist, intID_list, Order_specieID_list
    ...
    #v_initial = n_R0*R*T/p # m3, volume the reactor/batch when it is dependent on temp (T)
    v_initial = v_R0  # m3, volume the reactor/batch when it is dependent on temp (T)
    v_batch = v_initial #*1000 # m3, volume the reactor/batch
    reactor_model = [outflow_rate, v_batch, n_in, SA]
...
#=====================================================================================================
#=====================================================================================================
def solve_odes47(SA, n_R0,v_R0,C_R0,outflow_rate,T,tim,dydt,Gm,intID,specieID_list, S_x, fat, bat, gat):
    """
    Time-integrate chemo-kinetic system
    mc0 (gram)e.g.25, C_R0 (mol/m3)e.g.950, outflow_rate(m3/s)e.g. 5e-7, 
    Temp(K), time(sec), dydt, Energylist, intID_list, Order_specieID_list
    ...
    #S_x = S_x_in/(1e-6) # umol/m^2 (equivalent of 0.44 ML from expt data)
    #Input specifications for reactor model
    n_in = 0 # mol, amount of H2/P charged in at time, t
    ...
    # Initial conditions
    S_x0 = S_x
    y0 = [S_x0*1, S_x0*0, S_x0*0, S_x0*0, S_x0*0, S_x0*0, n_R0, 0, 0,    0]  # X, RX, UX, HX, Vx, PX, R, Hin, P, Hout
    t0 = 0
    t1 = tim # total integration time in sec
    ...
    # integrate on a logaritmic scale
    xx = np.linspace(-6, t1, 5)
    #xx = np.linspace(-6.0, np.log10(t1), int((np.log10(t1) + 24.0) * 100))    #(-14, np.log10(t1), int((np.log10(t1-500)+100)*200)) #int(t1*200))  #t1), 600)#
...
def solve_odes46cc(SA, n_R_init,v_R0,C_R0,outflow_rate,T,tim,dydt,Gm,intID,specieID_list, S_x, fat, bat, gat, initial_coverage):
    ...
    v_initial = v_R0*1  # in m3, volume the reactor/batch when it is dependent on temp (T)
    v_batch = v_initial*1 # mean 45000mL reactor-vol  #*15 # m3, volume the reactor/batch
    ...
    # construct ODE solver
    r = ode(dydts).set_integrator('vode', method='bdf', 
           atol=1e-8, rtol=1e-8, nsteps=100, with_jacobian=True)
    ...
    for x in xx:
        ...
        current=r.integrate(tnew)
        #""" 
        if (current[-4]/n_R0)<(1-0.6641):   # 0.00001(100%) / 0.80(20%) / 0.95(5%) for time selection
            time.append(tnew/60/60)
            print('the reaction time (t_max) was ',tnew/60/60,' hours')
            break
        #"""
...
#=====================================================================================================
#SEMI-BATCH MODEL (new)
#=====================================================================================================
def dydts(t, y, params):
    """
    ...
    #X = 1 - (RX/(X+RX+HX+UX+VX+PX) + UX/(X+RX+HX+UX+VX+PX) + HX/(X+RX+HX+UX+VX+PX) + VX/(X+RX+HX+UX+VX+PX) + PX/(X+RX+HX+UX+VX+PX))

    # ODEs for rate of change in gas-phase concentration
    dydt[6] = SA*(-rf1 + rb1) # R in mol/sec 
    dydt[7] = SA*(rf5 - rb5) + outflow_rate/v_batch*(n_in - n_H2)  # H2 in mol/sec
    dydt[8] = SA*(rf4 - rb4)  # P in mol/sec
    #dydt[9] = 0#-outflow_rate/v_batch*(n_in - n_H2)  # inert/nothing in mol/sec
...
def dydtc(t, y, params):
    ...
    n_out = 0 # mol, amount of R charged out at time, t     
    dydt[6] = SA*1e-6*(-rf1 + rb1) + outflow_rate/v_batch*(n_R - n_out) # R in mol/sec 
    dydt[7] = SA*1e-6*(rf5 - rb5) + outflow_rate/v_batch*(n_in - n_H2)  # H2 in mol/sec
    dydt[8] = SA*1e-6*(rf4 - rb4) + outflow_rate/v_batch*(n_in - n_P)  # P in mol/sec
