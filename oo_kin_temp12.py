...
def solve_odes46c(SA, n_R0,v_R0,C_R0,outflow_rate,T,tim,dydt,Gm,intID,specieID_list, S_x, fat, bat, gat):
    ...
    v_batch = v_initial*1 #1000 # mean 45000mL reactor-vol  #*15 # m3, volume the reactor/batch
    ...
    t0 = 0
    t1 = v_batch #tim # total integration time in sec
    ...
    for x in xx:
        ...
        """ 
        if (current[-3]/n_R0)<0.800000:   # 0.00001(100%) / 0.80(20%) / 0.95(5%) for time selection
...
def dydtc(t, y, params):
    ...
    dydt[0] = 1*(-rf1 + rb1 - rf2 + rb2 - rf3 + rb3 + rf4 - rb4  + 2*rf5 - 2*rb5)*ttt # X in mol/m2/sec
    dydt[1] = 1*(rf1 - rb1 - rf2 + rb2)*ttt # RX in mol/m2/sec
    dydt[2] = 1*(rf2 - rb2 - rf3 + rb3)*ttt # UX in mol/m2/sec
    dydt[3] = 1*(rf2 - rb2 + rf3 - rb3 - 2*rf5 + 2*rb5)*ttt # HX in mol/m2/sec
    dydt[4] = 1*(-rf6 + rb6)*ttt # VX in mol/m2/sec
    dydt[5] = 1*(rf3 - rb3 - rf4 + rb4 + rf6 - rb6)*ttt # PX in mol/m2/sec

    # ODEs for rate of change in gas-phase concentration (CSTR reactor model)
    n_out = 0 # mol, amount of R charged out at time, t 

    dydt[6] = SA*1e-6*(-rf1 + rb1)*ttt + 0.1*outflow_rate*(n_R - n_out)*ttt # R in mol/sec 
    dydt[7] = SA*1e-6*(rf5 - rb5)*ttt + outflow_rate*(n_in - n_H2)*ttt  # H2 in mol/sec
    dydt[8] = SA*1e-6*(rf4 - rb4)*ttt + outflow_rate*(n_in - n_P)*ttt  # P in mol/sec
