...
from sb_kin_temp0 import kineticparameter, dydts,solve_odes,solve_odes2,solve_odes3,solve_odes4,solve_odes46cc #40
import numpy as np
...
def EaApp(moleculer_property_function, title_of_molecule, Order_specieID_list, rtime, rtemp, cmass, flowout,initial_coverage):      # <=============  
    ...
    outflow_rate = flowout * 0.5e-3 #5.5e-12# 1 # 1.5 # in m3/sec ======================================================================14.27*1e-7 (original)
...
#=====================
    ...
    def EA_effect(moleculer_property_function, title_of_molecule, Order_specieID_list, fat, bat, gat):      # <=============  
        ...
        #TT = [temp-20,temp-10,temp,temp+10,temp+20] # USING +8/-20K (exclude last 3 pt)
        ...
        for i in range(tlens):
            ...
            #fig.savefig('/home/toyegoke/ENS_kinetics/Final-Ea/'+title+'_'+'_plot_data_for_COMBINED_ENERGY_PROFILE.png')  # +str(T)
            #fig2.savefig('/home/toyegoke/ENS_kinetics/Final-Ea/'+title+'_'+'_plot_data_for_ENERGY_PROFILE.png')  #  +str(T)  
            ...
            tt0, y= solve_odes46cc(SA,n_R0,v_R0,C_R0,outflow_rate,T,TIME,dydts, G_m, int_id, Order_specieID_list,S_x, fat, bat, gat, initial_coverage)
            ...
            rP = SA*1e-6*kf4 * PX / n_R0  # in a sec
            rH = SA*1e-6*kf5 * HX**2 /n_R0  # in a sec
            ...
            rP1 = SA*1e-6*(kf4 * PX - kb4*n_P * X)/n_R0  # in sec
            rH1 = SA*1e-6*(kf5 * HX**2 - kb5*n_H2 * X**2)/n_R0  # in sec
            ...
            RHH.append((n_H2/n_Ro - n_H2b/n_Ro)/(Ttim - Ttimb)) # 1/s
...
tim = 3600*1 # since the values is constant for over 10**5
