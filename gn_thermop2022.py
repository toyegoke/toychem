...
def thermoppt(CONTCAR_linK, OUTCAR_linK, SYMMERY_NO, Temperature_in_K, specie_id, ANY_OTHER_LINK = None):
    ...
    #========================================================
    if specie_id == 'X':
        STATUS = 'surf' # catalyst site in form of a specie
        ...
        if 'X' in yy:
            ...
            STATUS = 'ads' # adsorbate specie
            print(specie_id,' is the adsorbate specie')
        elif twoletters == 'ts' or 'TS' in yy:
            STATUS = 'ts' # catalyst site in form of a specie
            print(specie_id,' is the catalyst site specie')
        else:
            STATUS = 'nads' # gas species
            print(specie_id,' is the gas species')
    ...
    if os.path.isfile(b3):
        ...
        print ("It is zipped file that exist")
        file= gzip.open(b3+'.gz', 'rb')  # OUTCAR in freq file link
        f= gzip.open('column', 'w+')
        lines = file.readlines() # in the file link
        i=0
        nop=len(lines)
        print('nop=',nop)
    ...
    if elec is None:
        ...
        #print(elect[0], 'elect0=')
        print ('there is electronic contribution on the surface')
        E_elect=elect[-1]*96485 # in J/mol (1ev = 96485 J/mol)
        #print(E_elect, 'E_elect=')
    ...
    if len(wv_nm) == 0:
        print('no vibration freq available')
    else:
        print('vibration freq available')               
    #print('wv_nm=',wv_nm)
    ...
    if int(dof)=0:    
        wv = [0] 
        print ('CAT/ADS/GAS(nads) = no vibrational effect on the surface ')
    else:
        print ('CAT/ADS/GAS(nads) = there is vibrational effect on the surface')
        int1=int(3*sum1)
        int2=int(dof[0])
        print('int1=',int1)
        print('int2=',int2)
        ...
        if STATUS=='nads':
            ...
            if img_no>0:
                ...
                for i in range(nop):
                    ...
                    #print(wv)
    if len(wv) ==0:
        print("no FREQ in the calculation")
    else:
        print('there is FREQ in the calc with no of freq: '+str(len(wv)))
    #print('wv=',wv)
    ...
    if STATUS=='surf':
        ...
        cp_t=0
        dH_t=0
        print ('CAT = no translational effect')
    elif STATUS=='nads':
        print ('GAS = there is translational effect')
        #print('molecules')
        #for s_t
        s_t = R*((3/2)*(np.log(2*pi*m/h/h)) + (5/2)*(np.log(kB*T)) - (np.log(p)) + 5/2)
        s_t2 = R*((3/2)*(np.log(2*pi*m*kB*T/h/h)) + (np.log(kB*T/p)) + 5/2)
        #print('s_t2 is NEW CAL in j/mol/K =',s_t2, 's_t2 is NEW CAL in ev/K =',s_t2/96485)
        s_t3 = R*((3/2)*(np.log(2*pi*m*kB*T/h/h)) + (np.log(v/N)) + 5/2)
        #print('s_t3 is NEW CAL in j/mol/K =',s_t3, 's_t3 is NEW CAL in ev/K =',s_t3/96485)
        #P=1 #bar
        #s_t=R*( (5/2)*np.log(T)   +   (3/2)*np.log(tot_mm)  - np.log(P) - 1.1516)
        #for q_t2
        q_t=((2*pi*m*kB*T)**(3/2))/h/h/h* (kB*T/p)
        q_t2=((2*pi*m*kB*T/h**2)**(3/2))* (kB*T/p)
        q_t3=((2*pi*m*kB*T/h**2)**(3/2))* v/N
        sst= kB * (5./2. + np.log(q_t2)) * N 
        #print('sst(J/K)=',sst,'sst/96485(eV/K)=',sst/96485)
        #print('qt=', q_t,'qt2=', q_t2,'qt3=', q_t3,'v=', v,'m=',m,'h=',h,'kB=',kB,'T=', T)
        #for cp_t
        cp_t=(5/2)*R
        #for dH_t
        dH_t=(5/2)*R*T
    elif STATUS=='ads':
        print ('NON-CAT = no translational effect')
        #print('surfaces')
        s_t=0
        q_t=0
        cp_t=0
        ...
        print ('TS = no translational effect')
        #print('surfaces')
        s_t=0
        q_t=0
        cp_t=0
    ...
    if STATUS=='surf':
        ...
        dH_r=0
        tetha_rot=0
        print ('CAT = no rotational effect')
    elif STATUS=='nads':
        print ('GAS = there is rotational effect')
        #print('molecules')
        if Itest1 or Itest2 or Itest3:
            #print('linear')
        ...
        print ('ADS = there is rotational effect')
        Ia=0
        Ib=0
        Ic=0
        q_r=0
        ...
        print ('TS = there is rotational effect')
        Ia=0
        Ib=0
        Ic=0
        q_r=0
    ...
    if STATUS=='surf':
        q_v=0; s_v=0; cp_v=0; dH_v=0
        print ('CAT = no vibrotional effect')
    else:   
        nop=len(vi)
        print('No. of FREQ =',nop)
        if nop ==0:
            print ('GAS/ADS/TS = no vibrational effect')
            s_v=0; cp_v=0; dH_v=0; q_v=0 
        else:
            print ('GAS/ADS/TS = there is vibrational effect')
            for i in range(nop):
                #for list of vi
                ...
                dH_v0=R*T*(tetha_v0)/(np.exp(tetha_v0) - 1) # from Essential of compt chemistry book
                dH_v1.append(dH_v0)
    ...
    print('S=',S,'J/K/mol','    Cp=',Cp,'J/K/mol','    dH=',dH,'J/mol' , '    G=',G/96485,'eV', 'Elec=',E_elect/96485,'eV')
    print()
    #print('&s_t(J/mol/K)&',s_t,'&s_r(J/mol/K)&',s_r,'s_v(J/mol/K)&',s_v,'&E_elect(J/mol)&',E_elect,'&ZPE(J/mol)&',ZPE)
    #print('&s_t(eV/K)&',s_t/96485,'&s_r(eV/K)&',s_r/96485,'s_v(eV/K)&',s_v/96485,'&E_elect(eV)&',E_elect/96485,'&ZPE(eV)&',ZPE/96485)
    #print('st in j/mol/k', st0)
    #print()
    #print('qt=',q_t,'qr=',q_r,'qv=',q_v)
    #print()    
    #print('dH_t=',dH_t, 'dH_r=',dH_r, 'dH_v=',dH_v)
    #print('dH_t=',dH_t/96485, 'dH_r=',dH_r/96485, 'dH_v=',dH_v/96485)
    print('qt=',q_t,'dH_t=',dH_t/96485,'&s_t(eV/K)&',s_t/96485)
    print()    
    print('qr=',q_r,'dH_r=',dH_r/96485,'&s_r(eV/K)&',s_r/96485)
    print()
    print('qv=',q_v, 'dH_v=',dH_v/96485,'s_v(eV/K)&',s_v/96485)
    print('=====================')  
    print('end of calc for the species')
    print('=====================')
...
def strans2D(mtot,T,A):
    ...
    pi=math.pi 
    R=N*kB #const.gas_constant # 8.314462618 J mol^-1 K^-1
    qtrans2D = (2 * pi * mtot * kB * T / (h**2)) * A
    Etrans2D = kB * T 
    Strans2D = kB * (2. + np.log(qtrans2D))
    #self.S['trans2D'] = kB * ((5./3.) + np.log(qtrans2D))
    return Etrans2D,Strans2D

def ads_TS2d(X_gas, Xt_gas, Xt_2D):
    """
    X_gas can be either S or H depending properties of interest for gas	
    Xt_gas is the gas translation contribution to the concerned properties which must be similar with X_gas
    """
    X_TS = (X_gas - Xt_gas) + Xt_2D   # lossing 1/3 of its translation effect to give us the TS-state property
    print ("X_TS = ", X_TS)
    return X_TS  

def adsTS_E2d(gas_specie_id,surf_specie_id,ads_specie_id, gas_mol, surf_mol, ads_mol, Temperature_in_K, G_m, H_m, dH_t_m, S_m, s_t_m,int_id,tot_mm,A):
    """
    1-gas_specie_id,
    2-surf_specie_id,
    3-ads_specie_id,
    ...
    """
    T= Temperature_in_K
    mtot = tot_mm[specie_index(gas_specie_id,int_id)]
    [Etrans2D,Strans2D]=strans2D(mtot,T,A)
    
    H_gasTS = ads_TS2d(H_m[specie_index(gas_specie_id,int_id)], dH_t_m[specie_index(gas_specie_id,int_id)], Etrans2D)
    S_gasTS = ads_TS2d(S_m[specie_index(gas_specie_id,int_id)], s_t_m[specie_index(gas_specie_id,int_id)], Strans2D)
    G_gasTS = H_gasTS - T * S_gasTS
...
def PESdata(T,filename_link_fomat,energylist,int_id,x,r,rx,ux,hx,vx,px,h2,p,ts1,ts2,tsr,tsp,tsh2):
    ...
    #gas_phase
    gas_phase=(R+3*X) - (R+3*X)
    ZP_g.append(gas_phase/96485)    # in eV (converted from kJ/mol to eV)
    gas_phase=(R+3*X) - (R+3*X)
    ZP_g.append(gas_phase/96485)
    ...
    return ZP_g  # in eV (not kJ/mol)





def PESdatab(T,filename_link_fomat,energylist,int_id,x,r,rx,ux,hx,vx,px,h2,p,ts1,ts2,ts3,tsr,tsp,tsh2):
    ...
    TS1=energylist[specie_index(ts1,int_id)]
    TS2=energylist[specie_index(ts2,int_id)]
    TS3=energylist[specie_index(ts3,int_id)]
    TSR=energylist[specie_index(tsr,int_id)]
    TSP=energylist[specie_index(tsp,int_id)]
    TSH2=energylist[specie_index(tsh2,int_id)] 
    ...
    #gas_phase
    gas_phase=(2*R+4*X) - (2*R+4*X)
    ZP_g.append(gas_phase/96485)
    gas_phase=(2*R+4*X) - (2*R+4*X)
    ZP_g.append(gas_phase/96485)

    #TS_R_ads
    """  TS_RX=(TSR+3*X) - (2*R+4*X)  # the TSR already have X present 
    ZP_g.append(TS_RX/96485)
    TS_RX=(TSR+3*X) - (2*R+4*X)
    ZP_g.append(TS_RX/96485)    """

    TS_RX=gas_phase  # the TSR already have X present 
    ZP_g.append(TS_RX/96485)
    TS_RX=gas_phase
    ZP_g.append(TS_RX/96485)

    #print('(TSR+3*X)= ',int((TSR+3*X)/96485), '(R+4*X)=',  int((R+4*X)/96485) )

    #ads_R
    ads_react=(RX+3*X) - (2*R+4*X)
    ZP_g.append(ads_react/96485)
    ads_react=(RX+3*X) - (2*R+4*X)
    ZP_g.append(ads_react/96485)

    #TS1_deh2
    TS1_deh2=(TS1+3*X) - (2*R+4*X)
    ZP_g.append(TS1_deh2/96485)
    TS1_deh2=(TS1+3*X) - (2*R+4*X)
    ZP_g.append(TS1_deh2/96485)
    
    #deh2_intermediate
    interm=(UX+HX+2*X) - (2*R+4*X)
    ZP_g.append(interm/96485)
    interm=(UX+HX+2*X) - (2*R+4*X)
    ZP_g.append(interm/96485)

    #TS2_deh2
    TS2_deh2=(TS2+HX+2*X) - (2*R+4*X)
    ZP_g.append(TS2_deh2/96485)
    TS2_deh2=(TS2+HX+2*X) - (2*R+4*X)
    ZP_g.append(TS2_deh2/96485)

    #deh2_products
    ads_prod0=(VX+2*HX+X) - (2*R+4*X)
    ZP_g.append(ads_prod0/96485)
    ads_prod0=(VX+2*HX+X) - (2*R+4*X)
    ZP_g.append(ads_prod0/96485) 

    #TS3_deh2
    ads_prod=(TS3+2*HX+X) - (2*R+4*X)
    ZP_g.append(ads_prod/96485)
    ads_prod=(TS3+2*HX+X) - (2*R+4*X)
    ZP_g.append(ads_prod/96485)
    #print((ads_prod/96485))
    ...
    #TS_product_des
    """TS_PX=(TSP+3*HX) - (2*R+4*X) # the TSP already have X present 
    ZP_g.append(TS_PX/96485)
    TS_PX=(TSP+3*HX) - (2*R+4*X)
    ZP_g.append(TS_PX/96485)"""

    TS_PX=(P+3*HX+X) - (2*R+4*X) # the TSP already have X present 
    ZP_g.append(TS_PX/96485)
    TS_PX=(P+3*HX+X) - (2*R+4*X)
    ZP_g.append(TS_PX/96485)

    #print('(TSP+3*HX)=', int((TSP+3*HX)/96485), '(R+4*X)=',  int((R+4*X)/96485) )
    
    #product_des
    des_prod=(P+3*HX+X) - (2*R+4*X)
    ZP_g.append(des_prod/96485)
    des_prod=(P+3*HX+X) - (2*R+4*X)
    ZP_g.append(des_prod/96485)
    #print((des_prod/96485))
    ...
    #TS_H2_des
    """TS_h2=(3*TSH2+P+X) - (2*R+4*X)  # the TSH2 already have 2X present 
    ZP_g.append(TS_h2/96485)
    TS_h2=(3*TSH2+P+X) - (2*R+4*X)
    ZP_g.append(TS_h2/96485)"""

    TS_h2=(P+3*H2+4*X) - (2*R+4*X)  # the TSH2 already have 2X present 
    ZP_g.append(TS_h2/96485)
    TS_h2=(P+3*H2+4*X) - (2*R+4*X)
    ZP_g.append(TS_h2/96485)
    
    #print('(TSH2+0.5*P+4*X)=', int((TSH2+0.5*P+4*X)/96485), '(R+4*X)=',  int((R+4*X)/96485) )
    
    #H2_desorption
    des_h2=(P+3*H2+4*X) - (2*R+4*X)
    ZP_g.append(des_h2/96485)
    des_h2=(P+3*H2+4*X) - (2*R+4*X)
    ZP_g.append(des_h2/96485)

    int_id = ['R+3X','R+3X','TSR+2X','TSR+2X','RX+2X','RX+2X','TS1+2X','TS1+2X','UX+HX+X','UX+HX+X',
              'TS2+HX+X','TS2+HX+X','VX+2HX','VX+2HX','TS3+HX+X','TS3+HX+X','PX+2HX','PX+2HX','TSP+2HX','TSP+2HX','P+2HX+X','P+2HX+X',
              'TSH2+P+X','TSH2+P+X','P+H2+3X','P+H2+3X']
...
def PESdataHMF_no_h2o_DFF_Pd(T,filename_link_fomat,energylist,int_id,slab,hmf,O2g,xO2,xO,xhmf,xdff,xOH,xffca,xfdca,xH2O,fdca,H2O):

    #Free Gibb (G) energy for the species
    slab=energylist[specie_index(slab,int_id)]
    hmf=energylist[specie_index(hmf,int_id)]
    O2g=energylist[specie_index(O2g,int_id)]
    xO2=energylist[specie_index(xO2,int_id)]
    xO=energylist[specie_index(xO,int_id)]
    xhmf=energylist[specie_index(xhmf,int_id)]
    xdff=energylist[specie_index(xdff,int_id)]
    xOH=energylist[specie_index(xOH,int_id)]
    xffca=energylist[specie_index(xffca,int_id)]
    xfdca=energylist[specie_index(xfdca,int_id)]
    xH2O=energylist[specie_index(xH2O,int_id)]
    fdca=energylist[specie_index(fdca,int_id)]
    H2O=energylist[specie_index(H2O,int_id)] 
    ...
    #gas_phase
    gas_phase=(4*slab+hmf+1.5*O2g) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(gas_phase/96485)    # in eV (converted from kJ/mol to eV)
    gas_phase=(4*slab+hmf+1.5*O2g) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(gas_phase/96485)

    #TS_R_ads
    TS_RX=(2.5*slab+hmf+1.5*xO2) - (4*slab+hmf+1.5*O2g)  # the TSR already have X present 
    ZP_g.append(TS_RX/96485)
    TS_RX=(2.5*slab+hmf+1.5*xO2) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(TS_RX/96485)

    #ads_R
    ads_react=(1*slab+hmf+3*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(ads_react/96485)
    ads_react=(1*slab+hmf+3*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(ads_react/96485)

    #TS1_deh2
    TS1_deh2=(1*slab+hmf+3*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(TS1_deh2/96485)
    TS1_deh2=(1*slab+hmf+3*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(TS1_deh2/96485)
    
    #deh2_intermediate
    interm=(1*slab+hmf+3*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(interm/96485)
    interm=(1*slab+hmf+3*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(interm/96485)

    #TS2_deh2
    TS2_deh2=(0*slab+xhmf+3*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(TS2_deh2/96485)
    TS2_deh2=(0*slab+xhmf+3*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(TS2_deh2/96485)

    #deh2_products
    ads_prod0=(0*slab+xdff+2*xO+xH2O) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(ads_prod0/96485)
    ads_prod0=(0*slab+xdff+2*xO+xH2O) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(ads_prod0/96485) 

    #transformed_ads_product
    ads_prod=(1*slab+xffca+xO+xH2O) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(ads_prod/96485)
    ads_prod=(1*slab+xffca+xO+xH2O) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(ads_prod/96485)
    print((ads_prod/96485))
    ...
    #TS_product_des
    TS_PX=(2*slab+xfdca+xH2O) - (4*slab+hmf+1.5*O2g) # the TSP already have X present 
    ZP_g.append(TS_PX/96485)
    TS_PX=(2*slab+xfdca+xH2O) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(TS_PX/96485)
    
    #product_des
    des_prod=(3*slab+fdca+xH2O) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(des_prod/96485)
    des_prod=(3*slab+fdca+xH2O) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(des_prod/96485)

    #H2_desorption
    des_h2=(4*slab+fdca+H2O) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(des_h2/96485)
    des_h2=(4*slab+fdca+H2O) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(des_h2/96485)

    int_id = ['4*slab+hmf+1.5*O2g',' ','2.5*slab+hmf+1.5*xO2',' ','1*slab+hmf+3*xO',' ','1*slab+hmf+3*xO',' ',
              '1*slab+hmf+3*xO',' ','0*slab+xhmf+3*xO',' ','0*slab+xdff+2*xO+xH2O',' ','1*slab+xffca+xO+xH2O',' ',
              '2*slab+xfdca+xH2O',' ','3*slab+fdca+xH2O',' ','4*slab+fdca+H2O',' ']
...
def PESdataHMF_no_h2o_HMFCA_Pd(T,filename_link_fomat,energylist,int_id,slab,hmf,O2g,xO2,xO,xhmf,xhmfca,xOH,xffca,xfdca,xH2O,fdca,H2O):
    ...
    xO=energylist[specie_index(xO,int_id)]
    xhmf=energylist[specie_index(xhmf,int_id)]
    xhmfca=energylist[specie_index(xhmfca,int_id)]
    xOH=energylist[specie_index(xOH,int_id)]
    xffca=energylist[specie_index(xffca,int_id)]
    xfdca=energylist[specie_index(xfdca,int_id)]
    xH2O=energylist[specie_index(xH2O,int_id)]
    ...
    #deh2_products
    ads_prod0=(1*slab+xhmfca+2*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(ads_prod0/96485)
    ads_prod0=(1*slab+xhmfca+2*xO) - (4*slab+hmf+1.5*O2g)
    ZP_g.append(ads_prod0/96485) 
    ...
    int_id = ['4*slab+hmf+1.5*O2g',' ','2.5*slab+hmf+1.5*xO2',' ','1*slab+hmf+3*xO',' ','1*slab+hmf+3*xO',' ',
              '1*slab+hmf+3*xO',' ','0*slab+xhmf+3*xO',' ','1*slab+xhmfca+2*xO',' ','1*slab+xffca+xO+xH2O',' ',
              '2*slab+xfdca+xH2O',' ','3*slab+fdca+xH2O',' ','4*slab+fdca+H2O',' ']
...
def PESdataHMF_h2o_HMFCA_Au(T,filename_link_fomat,energylist,int_id,slab,hmf,O2g,xO2,xO,xhmf,xhmfca,xOH,xffca,xfdca,xH2O,fdca,H2O):
    ...
    #gas_phase
    gas_phase=(5*slab+hmf+1.5*O2g+1*H2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(gas_phase/96485)    # in eV (converted from kJ/mol to eV)
    gas_phase=(5*slab+hmf+1.5*O2g+1*H2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(gas_phase/96485)

    #TS_R_ads
    TS_RX=(3.5*slab+hmf+1.5*xO2+1*H2O) - (5*slab+hmf+1.5*O2g+1*H2O)  # the TSR already have X present 
    ZP_g.append(TS_RX/96485)
    TS_RX=(3.5*slab+hmf+1.5*xO2+1*H2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(TS_RX/96485)

    #ads_R
    ads_react=(2*slab+hmf+3*xO+1*H2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(ads_react/96485)
    ads_react=(2*slab+hmf+3*xO+1*H2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(ads_react/96485)

    #TS1_deh2
    TS1_deh2=(1*slab+hmf+3*xO+1*xH2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(TS1_deh2/96485)
    TS1_deh2=(1*slab+hmf+3*xO+1*xH2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(TS1_deh2/96485)
    
    #deh2_intermediate
    interm=(1*slab+hmf+2*xO+2*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(interm/96485)
    interm=(1*slab+hmf+2*xO+2*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(interm/96485)

    #TS2_deh2
    TS2_deh2=(0*slab+xhmf+2*xO+2*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(TS2_deh2/96485)
    TS2_deh2=(0*slab+xhmf+2*xO+2*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(TS2_deh2/96485)

    #deh2_products
    ads_prod0=(1*slab+xhmfca+1*xO+2*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(ads_prod0/96485)
    ads_prod0=(1*slab+xhmfca+1*xO+2*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(ads_prod0/96485) 

    #transformed_ads_product
    ads_prod=(1*slab+xffca+1*xH2O+2*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(ads_prod/96485)
    ads_prod=(1*slab+xffca+1*xH2O+2*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(ads_prod/96485)
    print((ads_prod/96485))
    ...
    #TS_product_des
    TS_PX=(2*slab+xfdca+2*xH2O) - (5*slab+hmf+1.5*O2g+1*H2O) # the TSP already have X present 
    ZP_g.append(TS_PX/96485)
    TS_PX=(2*slab+xfdca+2*xH2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(TS_PX/96485)
    
    #product_des
    des_prod=(3*slab+fdca+2*xH2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(des_prod/96485)
    des_prod=(3*slab+fdca+2*xH2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(des_prod/96485)

    #H2_desorption
    des_h2=(5*slab+fdca+2*H2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(des_h2/96485)
    des_h2=(5*slab+fdca+2*H2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(des_h2/96485)

    int_id = ['5*slab+hmf+1.5*O2g+1*H2O',' ','3.5*slab+hmf+1.5*xO2+1*H2O',' ','2*slab+hmf+3*xO+1*H2O',' ','1*slab+hmf+3*xO+1*xH2O',' ',
              '1*slab+hmf+2*xO+2*xOH',' ','0*slab+xhmf+2*xO+2*xOH',' ','1*slab+xhmfca+1*xO+2*xOH',' ','1*slab+xffca+1*xH2O+2*xOH',' ',
              '2*slab+xfdca+2*xH2O',' ','3*slab+fdca+2*xH2O',' ','5*slab+fdca+2*H2O',' ']
...
def PESdataHMF_h2o_DFF_Au(T,filename_link_fomat,energylist,int_id,slab,hmf,O2g,xO2,xO,xhmf,xdff,xOH,xffca,xfdca,xH2O,fdca,H2O):
    ...
    #deh2_products
    ads_prod0=(0*slab+xdff+4*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(ads_prod0/96485)
    ads_prod0=(0*slab+xdff+4*xOH) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(ads_prod0/96485) 
    ...
    des_h2=(5*slab+fdca+2*H2O) - (5*slab+hmf+1.5*O2g+1*H2O)
    ZP_g.append(des_h2/96485)
    print('FINAL=======================',des_h2/96485)

    int_id = ['5*slab+hmf+1.5*O2g+1*H2O',' ','3.5*slab+hmf+1.5*xO2+1*H2O',' ','2*slab+hmf+3*xO+1*H2O',' ','1*slab+hmf+3*xO+1*xH2O',' ',
              '1*slab+hmf+2*xO+2*xOH',' ','0*slab+xhmf+2*xO+2*xOH',' ','0*slab+xdff+4*xOH',' ','1*slab+xffca+1*xH2O+2*xOH',' ',
              '2*slab+xfdca+2*xH2O',' ','3*slab+fdca+2*xH2O',' ','5*slab+fdca+2*H2O',' ']
